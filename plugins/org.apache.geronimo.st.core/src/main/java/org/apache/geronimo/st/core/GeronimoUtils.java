...
public class GeronimoUtils {
    ...
    /**
     * Display the Eclipse Error log. Anything done to the UI in Eclipse must be performed 
     * in a UI thread. Since the GEP is not always in a UI thread we need to use the
     * asynchExec(Runnable) method to asynchronously start a UI thread.
     */
    public static void displayEclipseErrorLog() {
        final IWorkbenchPage activePage[] = new IWorkbenchPage[]{null};
        final IWorkbench workbench = PlatformUI.getWorkbench();
        workbench.getDisplay().asyncExec(new Runnable() {
             public void run() {
                 IWorkbenchWindow window = workbench.getActiveWorkbenchWindow();
                 if (window != null) {
                     activePage[0] = window.getActivePage();
                     try {
                         activePage[0].showView("org.eclipse.pde.runtime.LogView");
                     }
